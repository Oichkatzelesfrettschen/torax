...
from torax._src.core_profiles import (
    profile_conditions as profile_conditions_lib,
)
from torax._src.core_profiles.plasma_composition import (
    plasma_composition as plasma_composition_lib,
)
...
class RuntimeParamsProvider:
    ...
    def __call__(
        ...
        return runtime_params_lib.RuntimeParams(
            ...
            else self.edge.build_runtime_params(t),
        )
    ...
    def update_provider(
        ...
        return cast(
            typing_extensions.Self,
            eqx.tree_at(
                get_nodes_to_replace, self, replace=new_provider_values
            ),
        )
    ...
    def get_node_from_path(self, path: str) -> Any:
        ...
        for attr in attributes:
            try:
                ...
                raise ValueError(
                    f"Attribute {attr} of {path} not found."
                ) from exc
        return x
    ...
    def update_provider_from_mapping(
        ...
        return self.update_provider(
            get_replacements, tuple(replacements.values())
        )
...
def _get_provider_value_from_replace_value(
    ...
    match leaf:
        ...
        case _ if isinstance(leaf, (chex.Array, float)):
            if not isinstance(
                replace_value, (chex.Array, float, jax.core.Tracer)
            ):
                raise ValueError(
            ...
            if (
                leaf.shape != replace_value.shape
                or leaf.dtype != replace_value.dtype
            ):
                raise ValueError(
