...
class QLKNNTransportModel(pydantic_model_base.TransportBase):
    ...
    * If `model_name` is not set either, we load the default QLKNN model from
    ...
    def build_transport_model(
        self,
    ) -> qlknn_transport_model.QLKNNTransportModel:
...
class TGLFNNukaeaTransportModel(pydantic_model_base.TransportBase):
    ...
    model_name: Annotated[
        Literal["tglfnn-ukaea"], torax_pydantic.JAX_STATIC
    ] = "tglfnn-ukaea"
    machine: Annotated[
...
class ConstantTransportModel(pydantic_model_base.TransportBase):
    ...
    V_e: torax_pydantic.TimeVaryingArray = torax_pydantic.ValidatedDefault(
        -0.33
    )
...
class BohmGyroBohmTransportModel(pydantic_model_base.TransportBase):
    ...
    model_name: Annotated[
        Literal["bohm-gyrobohm"], torax_pydantic.JAX_STATIC
    ] = "bohm-gyrobohm"
    ...
    def build_runtime_params(
        ...
        return bohm_gyrobohm.RuntimeParams(
            ...
            chi_e_gyrobohm_multiplier=self.chi_e_gyrobohm_multiplier.get_value(
                t
            ),
            chi_i_bohm_multiplier=self.chi_i_bohm_multiplier.get_value(t),
            chi_i_gyrobohm_multiplier=self.chi_i_gyrobohm_multiplier.get_value(
                t
            ),
...
class CombinedTransportModel(pydantic_model_base.TransportBase):
    ...
    pedestal_transport_models: Sequence[CombinedCompatibleTransportModel] = (
        pydantic.Field(default_factory=list)
    )  # pytype: disable=invalid-annotation
    ...
    def _check_fields(self) -> typing_extensions.Self:
        ...
        if np.any(self.rho_min.value != 0.0) or np.any(
            self.rho_max.value != 1.0
        ):
            raise ValueError(
        ...
        if any(
            [
                np.any(model.rho_min.value != 0.0)
                or np.any(model.rho_max.value != 1.0)
                for model in self.pedestal_transport_models
            ]
        ):
            raise ValueError(
