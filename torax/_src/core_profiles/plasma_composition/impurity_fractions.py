...
class ImpurityFractions(torax_pydantic.BaseModelFrozen):
    ...
    def build_runtime_params(self, t: chex.Numeric) -> RuntimeParams:
        ...
        fractions = {
            ion: value.get_value(t) for ion, value in self.species.items()
        }
        fractions_face = {
        ...
        Z_override = (
            None if not self.Z_override else self.Z_override.get_value(t)
        )
        ...
        if not self.A_override:
            ...
            A_avg = jnp.full_like(
                fractions[ion_key], self.A_override.get_value(t)
            )
