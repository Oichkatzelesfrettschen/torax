...
class RuntimeParams:
    ...
    main_ion_names: tuple[str, ...] = dataclasses.field(
        metadata={"static": True}
    )
    impurity_names: tuple[str, ...] = dataclasses.field(
        metadata={"static": True}
    )
...
class PlasmaComposition(torax_pydantic.BaseModelFrozen):
    ...
    Z_eff: runtime_validation_utils.TimeVaryingArrayDefinedAtRightBoundaryAndBounded = torax_pydantic.ValidatedDefault(
        1.0
    )
    ...
    def _conform_impurity_data(cls, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(impurity_data, dict) and "impurity_mode" in impurity_data:
            if (
                Z_impurity_override is not None
                or A_impurity_override is not None
            ):
                logging.warning(
    ...
    def _check_zeff_usage(self) -> typing_extensions.Self:
        ...
        if (
            isinstance(
                self.impurity, electron_density_ratios.ElectronDensityRatios
            )
