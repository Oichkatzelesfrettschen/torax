...
class _ToricNN(nn.Module):
    ...
    def __call__(
        ...
        x = (
            x @ self.pca_components + self.pca_mean
        )  # Project back to true values.
...
class IonCyclotronSourceConfig(base.SourceModelBase):
    ...
    frequency: torax_pydantic.TimeVaryingScalar = (
        torax_pydantic.ValidatedDefault(120e6)
    )
    ...
    mode: Annotated[
        source_runtime_params_lib.Mode, torax_pydantic.JAX_STATIC
