...
from torax._src.neoclassical.transport import (
    runtime_params as transport_runtime_params,
)
...
def _calculate_Lmn(
    ...
    Lmn_e = Lmn_e.at[:, 0, 0].set(
        Kmn_e[:, 0, 0] * Ld * geo.gm4_face * geo.B_0**2
    )
    Lmn_e = Lmn_e.at[:, 0, 1].set(
        Kmn_e[:, 0, 1] * Ld * geo.gm4_face * geo.B_0**2
    )
    Lmn_e = Lmn_e.at[:, 0, 2].set(Kmn_e[:, 0, 2] * Lb)
    Lmn_e = Lmn_e.at[:, 0, 3].set(
        Kmn_e[:, 0, 3] * Ld / geo.gm5_face * geo.B_0**2
    )

    Lmn_e = Lmn_e.at[:, 1, 0].set(Lmn_e[:, 0, 1])
    Lmn_e = Lmn_e.at[:, 1, 1].set(
        Kmn_e[:, 1, 1] * Ld * geo.gm4_face * geo.B_0**2
    )
    Lmn_e = Lmn_e.at[:, 1, 2].set(Kmn_e[:, 1, 2] * Lb)
    Lmn_e = Lmn_e.at[:, 1, 3].set(
        Kmn_e[:, 1, 3] * Ld / geo.gm5_face * geo.B_0**2
    )
    ...
    Lmn_e = Lmn_e.at[:, 3, 3].set(
        Kmn_e[:, 3, 3] * Ld / geo.gm5_face * geo.B_0**2
    )
