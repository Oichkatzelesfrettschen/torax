...
class ExtendedLengyelModel(base.EdgeModel):
    ...
    def __call__(
        ...
        assert isinstance(edge_params, RuntimeParams), (
        ...
        mean_ion_charge_state = separatrix_electron_density / (
            n_i_sep + n_imp_sep
        )
        average_ion_mass = (
        ...
        fixed_impurity_concentrations = (
            edge_params.fixed_impurity_concentrations
        )
...
def _resolve_geometric_parameters(
    ...
    broadening = jnp.where(
        diverted, edge_params.divertor_broadening_factor, 1.0
    )
