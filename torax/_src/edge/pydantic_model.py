...
class ExtendedLengyelConfig(base.EdgeModelConfig):
    ...
    sheath_heat_transmission_factor: torax_pydantic.PositiveTimeVaryingScalar = torax_pydantic.ValidatedDefault(
        extended_lengyel_defaults.SHEATH_HEAT_TRANSMISSION_FACTOR
    )
    fraction_of_P_SOL_to_divertor: torax_pydantic.UnitIntervalTimeVaryingScalar = torax_pydantic.ValidatedDefault(
    ...
    mach_separatrix: torax_pydantic.NonNegativeTimeVaryingScalar = (
        torax_pydantic.ValidatedDefault(
            extended_lengyel_defaults.MACH_SEPARATRIX
        )
    )
    ...
    def build_runtime_params(
        ...
        fixed_impurity_concentrations = {
            k: v.get_value(t)
            for k, v in self.fixed_impurity_concentrations.items()
        }

        enrichment_model_multiplier = (
            self.enrichment_model_multiplier.get_value(t)
        )
        ...
        return extended_lengyel_model.RuntimeParams(
            ...
            divertor_broadening_factor=self.divertor_broadening_factor.get_value(
                t
            ),
