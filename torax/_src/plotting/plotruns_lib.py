...
class PlotProperties:
    ...
    legend_fontsize: int | None = (
        None  # None reverts to default matplotlib value
    )
...
class FigureProperties:
    ...
    def __post_init__(self):
        if len(self.axes) > self.rows * self.cols:
            raise ValueError(
                "len(axes) in plot_config is more than rows * columns."
            )
        ...
        if key in ds:
            ...
            return (
                ...
                if grid_type == "cell"
                    ...
                    (len(time), len(ds[output.RHO_FACE_NORM].to_numpy()))
                )
            )
...
def _update(
    ...
    def update_lines(plotdata, lines):
        ...
        for (
            cfg
        ) in plot_config.axes:  # Iterate through axes based on plot_config
        ...
        if cfg.plot_type == PlotType.SPATIAL:
            for attr, label in zip(cfg.attrs, cfg.labels):
                ...
                (line,) = ax.plot(
                    ...
                    plot_config.colors[line_idx % len(plot_config.colors)]
                    + dashed,
