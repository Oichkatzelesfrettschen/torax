...
class EQDSKConfig(torax_pydantic.BaseModelFrozen):
    ...
    geometry_type: Annotated[
        Literal["eqdsk"], torax_pydantic.TIME_INVARIANT
    ] = "eqdsk"
...
def _construct_intermediates_from_eqdsk(
    ...
    for n, (x_surface, z_surface) in enumerate(surfaces):
        ...
        surface_delta_upper_face = (
            R_major_local - X_upperextent
        ) / a_minor_local
        surface_delta_lower_face = (
            R_major_local - X_lowerextent
        ) / a_minor_local
        ...
        flux_surf_avg_grad_psi2_over_R2[n + 1] = (
            surface_FSA_abs_grad_psi2_over_R2
        )
...
def _validate_eqdsk_cocos11(eqfile: Mapping[str, np.ndarray | float]) -> None:
    ...
    if eqfile["psibdry"] < eqfile["psimag"]:
        COCOS_violations += (
            "- psi at the boundary is less than psi on the axis\n"
        )
