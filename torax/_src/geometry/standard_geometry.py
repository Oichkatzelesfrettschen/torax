...
class StandardGeometryIntermediates:
    ...
    def __post_init__(self):
        ...
        if self.diverted or self.flux_surf_avg_grad_psi2_over_R2[-1] < 1e-10:
            ...
            spline = lambda rho, data, x, bc_type: (
            ...
            self.flux_surf_avg_grad_psi[-1] = set_edge(
                self.flux_surf_avg_grad_psi
            )
            self.flux_surf_avg_grad_psi2[-1] = set_edge(
                self.flux_surf_avg_grad_psi2
            )
...
def build_standard_geometry(
    ...
    g2g3_over_rhon_hires = rhon_interpolation_func(
        rho_hires_norm, g2g3_over_rhon
    )
    g2g3_over_rhon = rhon_interpolation_func(rho_norm, g2g3_over_rhon)

    gm4 = rhon_interpolation_func(
        rho_norm, intermediates.flux_surf_avg_1_over_B2
    )
