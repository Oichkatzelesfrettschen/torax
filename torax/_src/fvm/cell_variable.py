...
class CellVariable:
    ...
    def face_centers(self) -> jt.Float[chex.Array, "face"]:
        ...
        return jnp.linspace(
            0.0, len(self.value) * self.dr, num=len(self.value) + 1
        )
    ...
    def face_grad(
        ...
        """
        if x is None:
            forward_difference = jnp.diff(self.value) / jnp.diff(
                self.cell_centers
            )
        else:
    ...
    def __str__(self) -> str:
        ...
        if self.right_face_grad_constraint is not None:
            output_string += f", right_face_grad_constraint={self.right_face_grad_constraint}"
        output_string += ")"
