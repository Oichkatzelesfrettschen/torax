...
def _extrapolate_face_profile_to_axis(
    ...
    clamp_rho_norm_value = jax.lax.dynamic_index_in_dim(
        geo.rho_norm, i, axis=-1
    )
    return jnp.where(
...
def j_parallel_to_j_toroidal(
    ...
    integral_face = jnp.concatenate(
        [
            ...
            math_utils.cumulative_cell_integration(
                integrand_cell * geo.rho_b, geo
            ),
        ]
    )
    ...
    S_face = jnp.concatenate(
        [
            jnp.array([0.0]),
            math_utils.cumulative_area_integration(
                jnp.ones_like(geo.rho_norm), geo
            ),
        ]
    )
