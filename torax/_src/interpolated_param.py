...
def convert_input_to_xs_ys(
    ...
    if isinstance(interp_input, xr.DataArray):
        ...
        return (
            np.asarray(
                interp_input.coords["time"], dtype=jax_utils.get_np_dtype()
            ),  # pytype: disable=attribute-error
            np.asarray(
                interp_input.values, dtype=jax_utils.get_np_dtype()
            ),  # pytype: disable=attribute-error
            interpolation_mode,
            is_bool_param,
        )
    ...
    if isinstance(interp_input, dict):
        ...
        return (
            np.array(
                list(interp_input.keys()), dtype=jax_utils.get_np_dtype()
            ),  # pytype: disable=attribute-error
            np.array(
                list(interp_input.values()), dtype=jax_utils.get_np_dtype()
            ),  # pytype: disable=attribute-error
            interpolation_mode,
            is_bool_param,
        )
...
class InterpolatedVarSingleAxis(InterpolatedParamBase):
    ...
    def __init__(
        ...
        """
        xs, ys = value
        xs = jax_utils.error_if(
            xs, jnp.any(jnp.diff(xs) < 0), "xs must be sorted."
        )
