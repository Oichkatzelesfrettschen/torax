...
class Grid1D(model_base.BaseModelFrozen):
    ...
    nx: typing_extensions.Annotated[
        pydantic.conint(ge=4), model_base.JAX_STATIC
    ]
...
class TimeVaryingArray(model_base.BaseModelFrozen):
    ...
    def get_value(
        ...
        match grid_type:
            ...
            case "face_right":
                return self.get_cached_interpolated_param_face_right.get_value(
                    t
                )
            case _:
    ...
    def update(
        ...
        if (
            replace_value.rho_norm is not None
            and replace_value.value is not None
        ):
            logging.info(
