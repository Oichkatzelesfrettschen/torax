...
class TimeVaryingScalar(model_base.BaseModelFrozen):
    ...
    def update(
        ...
        value = (
            replacements.value if replacements.value is not None else self.value
        )
    ...
    def _ensure_consistent_arrays(self) -> typing_extensions.Self:
        ...
        if self.time.dtype != self.value.dtype:
            raise ValueError(
                "The time and value arrays must have the same dtype."
            )

        if len(self.time) != len(self.value):
            raise ValueError(
                "The value and time arrays must be the same length."
            )
    ...
    def _conform_data(
        ...
        default_interpolation_mode = cls.model_fields[
            "interpolation_mode"
        ].default
