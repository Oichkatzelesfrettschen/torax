...
class BaseModelFrozen(pydantic.BaseModel):
    ...
    def time_invariant_fields(cls) -> tuple[str, ...]:
        ...
        return tuple(
            ...
            if TIME_INVARIANT in v.metadata
        )
    ...
    def _direct_submodels(self) -> tuple[Self, ...]:
        ...
        leaves = {
            k: self.__dict__[k] for k in self.__class__.model_fields.keys()
        }
    ...
    def _update_fields(self, x: Mapping[str, Any]):
        ...
        for path, value in x.items():
            ...
            try:
                cfg = (
                    None if issubclass(field_type, pydantic.BaseModel) else cfg
                )
            except TypeError:
                pass
            value = pydantic.TypeAdapter(
                field_type, config=cfg
    ...
    def _lookup_path(self, paths: Sequence[str]) -> Self:
        ...
        if not isinstance(value, BaseModelFrozen):
            raise ValueError(
                f"The value at path {paths} is not a Pydantic model."
            )
        return value
