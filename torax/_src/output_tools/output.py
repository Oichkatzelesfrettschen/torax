...
class StateHistory:
    ...
    def __init__(
        ...
        self._stacked_core_profiles: state.CoreProfiles = (
            jax.tree_util.tree_map(stack, *self._core_profiles)
        )
        self._stacked_core_sources: source_profiles_lib.SourceProfiles = (
            jax.tree_util.tree_map(stack, *self._core_sources)
        )
        self._stacked_core_transport: state.CoreTransport = (
            jax.tree_util.tree_map(stack, *self._transport)
        )
        self._stacked_post_processed_outputs: post_processing.PostProcessedOutputs = jax.tree_util.tree_map(
    ...
    def simulation_output_to_xr(self) -> xr.DataTree:
        ...
        profiles_dict = {
            ...
            if v is not None
            and v.values.ndim > 1  # pytype: disable=attribute-error
        }
        ...
        scalars_dict = {
            ...
            for k, v in flattened_all_core_data.items()
            if v is not None
            and v.values.ndim in [0, 1]  # pytype: disable=attribute-error
        }
    ...
    def _pack_into_data_array(
        ...
        is_cell_plus_boundaries_var = lambda x: (
            x.ndim == 2
            and x.shape
            == (
                len(self.times),
    ...
    def _save_core_profiles(
        ...
        for attr_name in core_profiles_names:
            ...
            if attr_name == "A_impurity":
                ...
                is_constant = np.all(
                    attr_value == attr_value[..., 0:1], axis=-1
                )
                if np.all(is_constant):
                    ...
                    # Save as a profile.
                    face_value = stacked_core_profiles.A_impurity_face
                    data_to_save = extend_cell_grid_to_boundaries(
    ...
    def _save_core_sources(
        ...
        for profile in self._stacked_core_sources.psi:
            xr_dict[f"j_parallel_{profile}"] = self._stacked_core_sources.psi[
                profile
            ]
    ...
    def _save_post_processed_outputs(
        ...
        for field in dataclasses.fields(self._stacked_post_processed_outputs):
            ...
            attr_value = getattr(
                self._stacked_post_processed_outputs, attr_name
            )
